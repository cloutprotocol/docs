...
This guide covers advanced memory management strategies for production INK agents.
...
INK supports multiple embedding providers:
...
const memories = await agent.recall("market analysis", {
  // Semantic options
  limit: 20,
  threshold: 0.6,
...
```typescript
// Query multiple aspects and merge
const technicalMemories = await agent.recall("technical implementation");
const emotionalMemories = await agent.recall("user sentiment");
const recentMemories = await agent.recall("recent events", { recency: "24h" });

// Deduplicate and rank
const allMemories = mergeAndRank([
  ...technicalMemories,
  ...emotionalMemories,
  ...recentMemories,
]);
...
```typescript
// When storing a memory caused by another
await agent.remember({
  type: "episodic",
  content: "User expressed frustration about slow response",
  importance: 0.8,
  causedBy: previousMemoryId,  // Auto-creates caused_by link
});

// Follow causal chains
const chain = await agent.memory.getCausalChain(memoryId);
// Returns ordered list of memories in causal sequence
```
