---
title: "Deployment"
description: "Deploy your INK agent to production"
---

This guide covers deploying INK agents to production environments.
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: agent
          image: your-registry/ink-agent:latest
          ports:
            - containerPort: 3000
          envFrom:
            - secretRef:
                name: ink-agent-secrets
          resources:
            requests:
              memory: "256Mi"
              cpu: "100m"
...
INK agents are typically singletons (one instance per agent identity). The Convex backend handles scaling automatically.
