...
|-----------|--------|-------|
| Agent runtime | Complete | `src/core/agent.ts` |
| Tick execution | Complete | `src/core/tick.ts` |
| Plugin system | Complete | Dependency resolution, lifecycle hooks |
| Configuration | Complete | Zod-validated, env loading |
| Memory store | Basic | CRUD operations only |
| Knowledge store | Basic | Text-based queries |
| Backend abstraction | Complete | Convex + in-memory |
| Terminal UI | Complete | React Ink components |
| CLI | Complete | `convex-agent` command |

### Not Implemented

| Feature | Priority | Dependency |
|---------|----------|------------|
| Embedding generation | High | Requires Gemini/OpenAI key |
| Memory deduplication | High | Requires embeddings |
| Memory consolidation | High | Requires embeddings |
| Memory linking | Medium | Requires dedup first |
| Self-reflection | Medium | Requires consolidation |
| Consciousness layer | Medium | Port from CLARK |
| LLM integration | High | Requires OpenRouter/model |
| Convex schema | High | Required for persistence |
