...
```typescript
// Fish morph - elongates horizontally
...
// Blend both morphs with easing
function blendMorphPoint(original, fishFactor, alienFactor) {
  const fishPoint = transformFishPoint(original);
  const alienPoint = transformAlienPoint(original);

  return {
    x: original.x * (1 - fishFactor - alienFactor)
       + fishPoint.x * fishFactor
       + alienPoint.x * alienFactor,
    // ... y, z similarly blended
  };
}
```
